...
def load_local_settings():
    ...
    try:
        ...
        if os.path.exists(settings_file):
            ...
            logger.info(f"✅ Cargadas {len(settings.get('Values', {}))} variables de entorno desde {settings_file}")
            return True
        else:
            logger.error(f"❌ Archivo {settings_file} no encontrado")
            return False
...
def test_rag_context():
    """Prueba la función de búsqueda vectorial usando Azure Search directamente"""
    logger.info("=== Probando búsqueda en Azure Search ===")
    ...
    try:
        ...
        logger.info(f"Azure Search Endpoint: {'✅ Configurado' if search_endpoint else '❌ No configurado'}")
        logger.info(f"Azure Search Key: {'✅ Configurado' if search_key else '❌ No configurado'}")
        logger.info(f"Azure Search Index: {'✅ Configurado' if search_index else '❌ No configurado'}")
        
        if not all([search_endpoint, search_key, search_index]):
            logger.warning("⚠️ Azure Search no está completamente configurado")
            return False
        
        # Import Azure Search SDK
        try:
            from azure.core.credentials import AzureKeyCredential
            from azure.search.documents import SearchClient
        except ImportError:
            logger.error("❌ Azure Search SDK no disponible")
            return False
        ...
        # Process results
        results_count = search_results.get_count()
        logger.info(f"✅ Búsqueda exitosa: {results_count} resultados encontrados")
        ...
        logger.error(f"❌ Error probando Azure Search: {e}")
        return False
...
def main():
    ...
    if not load_local_settings():
        logger.error("❌ No se pudo cargar la configuración local")
        return False
